...
- **房间制统计**：跟随本地玩家的游戏战斗状态，进入战斗开始新一轮，脱离战斗后冻结结果，保留到下一轮战斗。
...
- “房间”以本地玩家的 `IsInBattle` 状态变化为边界，并非按地图房间 ID 去重；显示的房间编号是插件本次加载以来的战斗序号。
...
2. 将反馈中的攻击者沿 `NetworkLeader` 关系追溯到 `PlayerAvatar`，按玩家网络 ID 累加伤害。
3. 轮询本地玩家 `IsInBattle` 决定战斗统计的开始与冻结，不使用空闲超时。
...
确认已安装 BepInEx 5，DLL 位于 `plugins` 中，配置中的 `ToggleKey` 未被更改。检查 `BepInEx/LogOutput.log` 是否有 `Sephiria Multiplayer DPS Meter v1.4.2 loaded` 或加载错误；其他模组也可能占用 F9。

**面板一直显示等待，或者没有队友的数据？**

先进入战斗并造成伤害。面板只显示已收到有效伤害反馈的玩家；客户端同步范围或版本不兼容可能导致记录缺失，参见上面的联机说明。
