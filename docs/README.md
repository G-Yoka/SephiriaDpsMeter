...
- 源码版本：`v1.5.3`（死亡与复活状态修复已通过自动测试，待游戏内验证）
- 当前正式版：`v1.5.1`
- Steam AppID：[2436940](https://store.steampowered.com/app/2436940/Sephiria/)
- 运行环境：Windows / BepInEx 5 / Unity Mono
- 兼容性：`v1.5.1` 已通过编译、自动测试与游戏内测试（2026-08-28）；`v1.5.3` 已通过编译与自动测试，待游戏内验证；后续游戏更新需重新确认
...
- **死亡后保留（v1.5.3）**：本地玩家死亡时冻结当前结果；复活过渡期间不会因临时脱离战斗而清空数据，若在同一房间复活则继续累计。进入新房间、切换楼层或正常开始新一轮战斗时仍会重置。
...
| 房间用时 | 当前房间战斗的现实经过时间；脱离战斗或死亡期间冻结显示，同房复活后继续；切换房间或楼层时另起一轮；房间无法识别时暂停统计 |
...
确认已安装 BepInEx 5，DLL 位于 `plugins` 中，配置中的 `ToggleKey` 未被更改。检查 `BepInEx/LogOutput.log` 是否有 `Sephiria Multiplayer DPS Meter v1.5.3 loaded`（源码构建）或 `v1.5.1 loaded`（正式版），以及加载错误；其他模组也可能占用 F9。
