...
- 源码版本：`v1.5.2`（死亡后保留数据修复已通过自动测试，待游戏内验证）
- 当前正式版：`v1.5.1`
- Steam AppID：[2436940](https://store.steampowered.com/app/2436940/Sephiria/)
- 运行环境：Windows / BepInEx 5 / Unity Mono
- 兼容性：`v1.5.1` 已通过编译、自动测试与游戏内测试（2026-08-28）；`v1.5.2` 已通过编译与自动测试，待游戏内验证；后续游戏更新需重新确认
...
- **死亡后保留（v1.5.2）**：本地玩家死亡时冻结当前结果；若在同一房间复活则继续累计，不清空已有总伤。进入新房间、切换楼层或正常开始新一轮战斗时仍会重置。
...
确认已安装 BepInEx 5，DLL 位于 `plugins` 中，配置中的 `ToggleKey` 未被更改。检查 `BepInEx/LogOutput.log` 是否有 `Sephiria Multiplayer DPS Meter v1.5.2 loaded`（源码构建）或 `v1.5.1 loaded`（正式版），以及加载错误；其他模组也可能占用 F9。
