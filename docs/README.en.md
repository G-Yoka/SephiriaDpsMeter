...
- **Retain results after death (v1.5.4):** death freezes the current result, and the temporary non-battle phase during respawn no longer clears it. A same-room respawn continues the existing total. A failed run keeps its result on the death screen, then clears it after returning to the multiplayer lobby.
- **Lobby isolation (v1.5.4):** room detection only runs during an active dungeon run. Attacking a training dummy in the multiplayer lobby does not start recording or leave the panel stuck on “Locating room.”
