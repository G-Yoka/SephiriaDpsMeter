...
- Source version: `v1.5.3` — death and respawn state handling passed automated checks; in-game verification pending.
...
- Compatibility: `v1.5.1` passed compilation, automated checks and in-game testing on 2026-08-28. `v1.5.3` passes compilation and automated checks; in-game verification is pending. Game updates may require compatibility checks.
...
- **Retain results after death (v1.5.3):** death freezes the current result, and the temporary non-battle phase during respawn no longer clears it. Respawning in the same room continues the existing total. A new room, floor or normal new battle still resets statistics.
...
Confirm BepInEx 5 is installed, the DLL is in `plugins`, and `ToggleKey` has not changed. Check `BepInEx/LogOutput.log` for `Sephiria Multiplayer DPS Meter v1.5.3 loaded` (source build), `v1.5.1 loaded` (stable release), or errors. Another mod may also use F9.
